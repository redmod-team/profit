...
@author: calbert
"""
from config import params_uq, order_uq
...
def get_eval_points():
    ...
    distribution = J(*params_uq.values())
...
if __name__ == '__main__':
    ...
    expansion = orth_ttr(order_uq, distribution)
    approx = fit_quadrature(expansion, nodes, weights, output)
    F0 = E(approx, distribution)
    dF = Std(approx, distribution)
    urange = params_uq['ksNO3denit'].range()
    vrange = params_uq['bioturbation'].range()
