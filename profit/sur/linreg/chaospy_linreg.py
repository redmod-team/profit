...
class ChaospyLinReg(LinearRegression):
    ...
    def __init__(self, model='monomial', order=None, **kwargs):
        super().__init__()
        self.Mdim = order
    ...
    def transform(self, X):
        ...
        Phi = self.model(self.Mdim, **self.model_params)
        if self.ndim > 0:
            for var in vars[1:]:
                poly = self.model(self.Mdim, **self.model_params)
                poly.names = (var,)
                Phi = chaospy.outer(Phi, poly)
        return Phi.flatten()(*X.T)
