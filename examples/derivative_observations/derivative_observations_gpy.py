...
k = GPy.kern.RBF( input_dim = d , lengthscale = l , variance = sigma_f**2 ) # we choose the squared exponential function as a kernel
...
m = GPy.models.MultioutputGP( X_list = [x , x_der ] , Y_list = [y , y_der ] , kernel_list = [ k , k_der ] , likelihood_list = [ gauss , gauss_der ]) # We are building our model " m "
...
plot_gp_vs_real(m , x_star , y_der_star_true , [ x.shape[0] , x_der.shape[0]] , title = ' Latent function derivatives ' , fixed_input = 1 , xlim = [0 ,8] , ylim = [ -6.5 ,6.5])
plot_gp_vs_real(m , x_star , y_star_true , [ x.shape[0] , x_der.shape[0]] , title = ' Latent function ' , fixed_input = 0 , xlim = [0 ,8] , ylim = [ -2.1 ,2.1])

m.optimize() # Optimisation of the parameters
print( m ) # Showing the model

plot_gp_vs_real(m , x_star , y_der_star_true , [ x.shape [0] , x_der.shape[0]] , title = ' Latent function derivatives ' , fixed_input = 1 , xlim = [0 ,8] , ylim = [ -1.1 ,1.1])
plot_gp_vs_real (m , x_star , y_star_true , [ x.shape[0] , x_der.shape[0]] , title = ' Latent function ' , fixed_input = 0 , xlim = [0 ,8] , ylim = [ -1.1 ,1.1])
