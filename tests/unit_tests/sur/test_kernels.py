...
def test_grad_nll():
    ...
    dl = 1e-6
    lm0 = l + [-0.5*dl, 0]
    flikm0 = negative_log_likelihood_cholesky(np.hstack([lm0, [sigma_f, sigma_n]]), xa, ya, K)
    lp0 = l + [0.5*dl, 0]
    flikp0 = negative_log_likelihood_cholesky(np.hstack([lp0, [sigma_f, sigma_n]]), xa, ya, K)
    l0m = l + [0, -0.5*dl]
    flik0m = negative_log_likelihood_cholesky(np.hstack([l0m, [sigma_f, sigma_n]]), xa, ya, K)
    l0p = l + [0, 0.5*dl]
    flik0p = negative_log_likelihood_cholesky(np.hstack([l0p, [sigma_f, sigma_n]]), xa, ya, K)
    ...
    ds2 = 1e-6
    flikm0 = negative_log_likelihood_cholesky(np.hstack([l, [sigma_f-0.5*ds2, sigma_n]]), xa, ya, K)
    flikp0 = negative_log_likelihood_cholesky(np.hstack([l, [sigma_f+0.5*ds2, sigma_n]]), xa, ya, K)
    flik0m = negative_log_likelihood_cholesky(np.hstack([l, [sigma_f, sigma_n-0.5*ds2]]), xa, ya, K)
    flik0p = negative_log_likelihood_cholesky(np.hstack([l, [sigma_f, sigma_n+0.5*ds2]]), xa, ya, K)
...
def test_optim():
    ...
    def cost(loghyp):
        h = 10**loghyp
        val, jac = negative_log_likelihood_cholesky(h, xtrain, ytrain, RBF, eval_gradient=True, log_scale_hyp=True)
        return val, h*np.log(10.0)*jac

    opt_res = minimize(cost, loghyp0, method='L-BFGS-B', jac=True, bounds=((-2, 2), (-2, 2), (-2, 2), (-4, 0)))
